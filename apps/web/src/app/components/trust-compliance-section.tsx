  ...
  <div className="relative z-10 max-w-6xl mx-auto">
    ...
    <div className="text-center mb-14">
      ...
      <h2 className="text-4xl font-bold text-velvet font-serif mb-3">
        Trust &amp; <span className="text-gold-dark">Compliance</span>
      </h2>
      <div className="w-14 h-0.5 bg-gold mx-auto" />
    </div>

    {/* Regulation cards */}
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-gold/10 mb-px">
      {regulations.map((r) => (
        <div key={r.title} className="bg-white hover:bg-pearl transition-colors p-10 flex gap-6 border border-gold/20">
          <div className="w-14 h-14 border border-gold bg-gold/10 flex items-center justify-center shrink-0">
            <svg className="w-6 h-6 fill-gold-dark" viewBox="0 0 24 24">
              <path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4z"/>
            </svg>
          </div>
          <div>
            <div className="text-gold-dark text-xs tracking-widest uppercase mb-1.5">{r.authority}</div>
            <h3 className="text-lg font-bold text-velvet font-serif mb-2">{r.title}</h3>
            <p className="text-xs text-velvet/70 leading-relaxed mb-3">{r.desc}</p>
            <span className="inline-block border border-gold/40 bg-gold/10 text-gold-dark text-xs tracking-widest px-3 py-1 uppercase">
              {r.pill}
            </span>
          </div>
        </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-gold/10 mb-px">
      {principles.map((p) => (
        <div key={p.title} className="bg-white hover:bg-pearl transition-colors p-7 flex gap-4 border border-gold/20">
          <div className="w-7 h-7 border border-gold/40 bg-gold/10 flex items-center justify-center shrink-0 mt-0.5">
            <svg className="w-3.5 h-3.5 fill-gold-dark" viewBox="0 0 24 24">
              <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/>
            </svg>
          </div>
          <div>
            <div className="text-sm font-bold text-velvet mb-1.5">{p.title}</div>
            <div className="text-xs text-velvet/70 leading-relaxed">{p.desc}</div>
          </div>
        </div>
      ))}
    </div>
    ...
    {/* Disclaimer */}
    <div className="border border-gold/30 border-t-2 border-t-gold bg-white px-8 py-5 flex gap-4 items-start">
      <svg className="w-5 h-5 fill-gold-dark shrink-0 mt-0.5" viewBox="0 0 24 24">
        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z"/>
      </svg>
      <p className="text-xs text-velvet/70 leading-relaxed">
        <span className="text-gold-dark font-semibold">Important Disclaimer: </span>
