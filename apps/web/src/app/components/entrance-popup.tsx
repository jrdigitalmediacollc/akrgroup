...
export function EntrancePopup() {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center" style={{ background: "#0e0201" }}>
      {/* Background image */}
      ...
      {["top-5 left-5 border-t border-l", "top-5 right-5 border-t border-r", "bottom-5 left-5 border-b border-l", "bottom-5 right-5 border-b border-r"].map((pos, i) => (
        <div key={i} className={`absolute w-14 h-14 border-amber-600 ${pos}`} />
      ))}

      {/* Card */}
      <div className="relative z-10 w-[480px] border border-amber-700/30 bg-[#1a0603] text-center overflow-hidden mx-4">
        <div className="h-0.5 bg-amber-600 w-full" />

        <div className="px-12 py-12">
          {/* Logo */}
          <div className="w-20 h-20 rounded-full border-2 border-amber-600 bg-amber-600/10 flex items-center justify-center mx-auto mb-5">
            <span className="text-xl font-bold text-amber-600 tracking-wider">AKR</span>
          </div>
          ...
          <button
            ...
            className="w-full bg-amber-600 hover:bg-amber-500 disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-bold tracking-[3px] uppercase py-4 transition-colors mb-3 flex items-center justify-center gap-2"
          >
            <svg className="w-4 h-4 fill-white" viewBox="0 0 24 24"><path d="M12 4l-1.41 1.41L16.17 11H4v2h12.17l-5.58 5.59L12 20l8-8z"/></svg>
            Enter Website
          </button>
        ...
        {/* Badges */}
        <div className="flex justify-center gap-5 px-8 py-3 border-t border-amber-700/15 bg-black/20">
        ...
        <div className="h-0.5 bg-amber-700/25" />
      </div>
    </div>
  );
}